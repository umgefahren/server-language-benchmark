...
impl FromStr for CompleteCommand {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        // println!("command => {:?}", s);
        let len = s.chars().filter(|c| *c == ' ').fold(1, |a, _| a + 1);
        let mut splits = s.split(' ');
        let command = match splits.next().ok_or("invalid command")? {
            "GET" => {
                if len != 2 {
                    return INVALID_COMMAND_ERR;
                }
                let key = splits.next().ok_or("invalid command")?;
                validate_string(key)?;
                CompleteCommand::Get {
                    key: key.to_string(),
                }
            ...
            "SET" => {
                ...
                let key = splits.next().ok_or(INVALID_COMMAND_STRING)?;
                validate_string(key)?;
                let value = splits.next().ok_or(INVALID_COMMAND_STRING)?;
            ...
            "DUMPINTERVAL" => {
                ...
                let duration_string = splits.next().ok_or(INVALID_COMMAND_STRING)?;
...
#[inline]
pub(crate) async fn write_record_opt<T: AsyncWrite + std::marker::Unpin>(
    record_opt: Option<Record>,
    mut writer: T,
    ...
    match record_opt {
        ...
        None => {
            ...
            writer
                .write_all((NOT_FOUND_STRING.to_owned() + "\n").as_bytes())
                .await?;
            writer.flush().await?;
        }
    }
...
pub(crate) async fn write_counter<T: AsyncWrite + std::marker::Unpin>(
    num: u64,
    mut writer: T,
) -> Result<(), Box<dyn Error>> {
...
pub(crate) async fn write_dump<T: AsyncWrite + std::marker::Unpin>(
    dump: Arc<String>,
    mut writer: T,
) -> Result<(), Box<dyn Error>> {
    let write_string = format!("{}\n", dump.as_str());
...
impl CompleteCommand {
    pub(crate) async fn execute<T: AsyncWrite + std::marker::Unpin>(
        &self,
        store: Arc<Store>,
        mut writer: T,
    ) -> Result<(), Box<dyn Error>> {
